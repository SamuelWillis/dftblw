defmodule DFTBLW.Mastery.Boundary.TemplateValidator do
  ...
  def errors(fields) when is_list(fields) do
    ...
    |> required(fields, :instructions, &validate_instructions/1)
  ...
  def validate_instructions(_instructions), do: {:error, "nust be a binary"}

  def validate_raw(raw) when is_binary(raw),
    do: raw |> String.match?(~r{\S}) |> check({:error, "can't be blank"})
